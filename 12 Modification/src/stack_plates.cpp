...
main (int argc, char **argv)
{
  ...
  SegmentPlane(cloud_sor_voxel,normal,coefficients_plane,inliers_plane,0.012);
  ...
  for(int i=0;i<infor.size();i++)
  {
    ...
    std::cout << " " <<std::endl;
    //std::cout << "direction : " << GetDirection(origin,infor[i].x,infor[i].y,infor[i].z,infor[i].radius,0.01) << std::endl;
  }
